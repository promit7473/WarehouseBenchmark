...
def publish_rgb(camera: Camera, freq):
    """Publish RGB camera data to ROS2."""
    # The following code will link the camera's render product and publish the data to the specified topic name.
    render_product = camera._render_product_path
    step_size = int(60/freq)
...
def publish_depth(camera: Camera, freq):
    """Publish depth camera data to ROS2."""
    # The following code will link the camera's render product and publish the data to the specified topic name.
    render_product = camera._render_product_path
    step_size = int(60/freq)
...
class goal_position(Node):
    """ROS2 node for publishing goal positions."""
...
class WarehousePose(Node):
    """ROS2 node for publishing warehouse robot pose."""
    def __init__(self, topic_name='robot_pose'):
        super().__init__('robot_pose')
        self.publisher_ = self.create_publisher(Pose, topic_name, 10)

    def publish_robot_position(self, robot_pose):
        pose = Pose()
        pose.position.x = float(robot_pose[0])
        pose.position.y = float(robot_pose[1])
        pose.position.z = float(robot_pose[2])

        pose.orientation.w = float(robot_pose[3])
        pose.orientation.x = float(robot_pose[4])
        pose.orientation.y = float(robot_pose[5])
        pose.orientation.z = float(robot_pose[6])
